...
func TestRestoreReplayHTTPTransport(t *testing.T) {
	for _, mode := range []string{"success", "HTTP error", "native error", "redirect", "late drift"} {
		t.Run(mode, func(t *testing.T) {
			...
			err = executeRestoreReplay(t.Context(), prepared, target, strings.Replace(server.URL, "http://", "ws://", 1), "root", DatabaseIdentity{Namespace: "phebs", Database: "phebs"}, nil)
			wantCalls := int32(3)
			if mode == "success" {
				wantCalls = 5
			}
...
func TestRestoreReplayHTTPCanceledAndEarlyRefusal(t *testing.T) {
	for _, canceled := range []bool{false, true} {
		t.Run(fmt.Sprint(canceled), func(t *testing.T) {
			...
			err = submitRestoreReplayUnit(ctx, &http.Client{Transport: transport}, server.URL, "spool-test-pass", DatabaseIdentity{Namespace: "neutral", Database: "neutral"}, file, unit, nil)
...
func TestRestoreReplayBootstrapFailureIsTerminal(t *testing.T) {
	...
	target := t.TempDir()
	for range 2 {
		if err := executeRestoreReplay(t.Context(), prepared, target, strings.Replace(server.URL, "http://", "ws://", 1), "root", DatabaseIdentity{Namespace: "phebs", Database: "phebs"}, nil); err == nil {
